...
if (window.location.href.includes('live.html')) {
    ...
    document.getElementById('run_out').addEventListener('click', function() {
        wicket = true;
        run_out = true;
        run = parseInt(prompt("How many runs were completed before run-out? (0, 1, 2, etc.)")) || 0;
        stump = (prompt("How many runs were completed before run-out? (0, 1, 2, etc.)")) || 0;
    ...
    function handle_balls() {
        if (!wicket) {
            ...
            total_wickets++;
            if (run_out) {
                // For run-outs, runs still count (unlike other dismissals)
                total_runs += run;
                update_batter_stats(strike_batter, run, wicket);
                update_bowler_stats(bowler, run, wicket);
                
                // Swap strike if odd runs (same as normal runs)
                if (run % 2 == 1) {
                    let temp = strike_batter;
                    strike_batter = non_strike_batter;
                    non_strike_batter = temp;
                ...
                // Existing wicket handling remains the same
                update_batter_stats(strike_batter, run, wicket);
                update_bowler_stats(bowler, run, wicket);
            }
            if(total_inning==1) {
