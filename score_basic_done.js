...
if (window.location.href.includes('setup.html')) {
    document.getElementById('start_match').addEventListener('click', function(event) {
        ...
        if (!team1_name_out || !team2_name_out || !toss_winner_out || !toss_decision_out) {
            ...
            let values_out = `?team1=${encodeURIComponent(team1_name_out)}&team2=${encodeURIComponent(team2_name_out)}&toss_winner=${encodeURIComponent(toss_winner_out)}&toss_decision=${encodeURIComponent(toss_decision_out)}`;
            localStorage.clear();
...
if (window.location.href.includes('live.html')) {
    ...
    function update_batter_stats(name, runs_scored, is_out = false,newb=false) {
        if (!batters[name]) {
            batters[name] = { runs: 0, balls: 0, fours: 0, sixes: 0, out: false, newb: true,};
        }
    ...
    function update_bowler_stats(name, runs_conceded, took_wicket = false) {
        if (!bowlers[name]) {
    ...
    function handle_balls() {
        if (!wicket) {
            ...
            else if(total_inning==2) {strike_batter = prompt("Enter the name of the next batter:") || "Player2 " + (total_wickets + 2);update_batter_stats(strike_batter, 0, 0,true);}
            if (balls_bowled % 6 == 0) {
...
if (window.location.href.includes("scorecard.html")) {
    ...
    Object.entries(batters_data).forEach(([name, stats]) => {
        ...
        row.innerHTML = `<td>${name}</td> <td>${stats.runs}</td> <td>${stats.balls}</td> <td>${stats.fours}</td> <td>${stats.sixes}</td> <td></td> <td>${stats.out ? 'Out' : 'Not Out'}</td>`;
    ...
    Object.entries(bowlers_data).forEach(([name, stats]) => {
        ...
        row.innerHTML = `<td>${name}</td> <td>${overs}</td> <td>${stats.runs}</td> <td>${stats.wickets}</td> <td>${stats.maidens}</td>`;
...
if (window.location.href.includes("summary.html")) {
    ...
    if(type==2) {
        const prev_wickets = matchState?.prev_wickets;
        let req_wickets=11-prev_wickets;
        const balls_bowled=matchState?.balls_bowled;
        let req_balls=6*OVERS-balls_bowled;
        document.getElementById("win").innerText=`${won_team} wins by ${req_wickets} wickets! (${req_balls} left)`;
    }
